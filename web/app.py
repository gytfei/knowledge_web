...
def ui_header():
    ...
    with col1:
        st.markdown("## 📚 Knowledge / Literature Memo (Web)")
    with col2:
        st.metric("Action Score", actionscore)
    with col3:
...
def ui_left_panel():
    ...
    if results:
        ...
        st.info("暂无结果（可以在右侧的“同义词管理/新建文档”中处理）")
...
def ui_right_panel(selected_db: str, root_path: str, doc_path: str):
    ...
    with colA:
        ref_num = st.text_input("Ref Num", value=st.session_state.get("ref_num", ""), key="ref_num")
    with colB:
        declare = st.text_input("Declare", value=st.session_state.get("declare", ""), key="declare")

    hold = st.checkbox("Hold Declare（保持不清空）", value=st.session_state.get("hold", False), key="hold")
    remove_enter = st.checkbox("Remove Enter（去掉换行）", value=False, key="remove_enter")
    ...
    editor_text = st.text_area("正文编辑区", value=st.session_state.get("editor_text", ""), height=260, key="editor_text")
    if st.button("清空"):
    ...
    if st.button("导入到当前 Word（docx）", key="btn_import_docx"):
        try:
            ...
            keyword = st.session_state.get("selected_content", "")
            # 你原来有 path_check 判断（keyword in word_path），这里简化为：doc_path 存在即允许
    ...
    # 图片导入（Web：用上传文件代替剪贴板图片）
    st.markdown("### 图片导入（替代剪贴板）")
    img_file = st.file_uploader("上传 PNG/JPG", type=["png", "jpg", "jpeg"], key="img_uploader")
    size = st.selectbox("图片缩放（size=1/2/3）", [1, 2, 3], index=0, key="img_size")

    if st.button("插入图片到当前 Word（docx）", key="btn_insert_img"):
        try:
            if not img_file:
                st.error("请先上传图片")
                return
...
def main():
    st.set_page_config(page_title="Knowledge Web", layout="wide")
